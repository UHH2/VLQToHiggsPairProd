...
for b in EtaPtBin :
    ...
    for h in listHistos :
        ...
        for tag in h.listTagger :
            ...
            #create final histos   
            if h.doPerformance :
                valHistos[keyHisto]=graphProducer(plot=h,histos=h_Val,color=mapColorStandard,isVal=True)
                if fileRef : refHistos[keyHisto]=graphProducer(plot=h,histos=h_Ref,color=mapColorStandard,isVal=False)
            else :    
                valHistos[keyHisto]=histoProducer(plot=h,histos=h_Val,keys=listFlavors,color=mapColorStandard,isVal=True)
                if fileRef : refHistos[keyHisto]=histoProducer(plot=h,histos=h_Ref,keys=listFlavors,color=mapColorStandard,isVal=False)
        ...
        if h.name=="FlavEffVsBEff_B_discr" :
            for f in ["C","DUSG"] :
                for isVal in [True] :
                    ...
                    #setup the histos
                    if isVal : Histos[keyHisto]=histoProducer(plot=h,histos=perfAll_Val[f],keys=perfAll_keys,color=mapColorStandard,isVal=isVal)
                    else : Histos[keyHisto]=histoProducer(plot=h,histos=perfAll_Ref[f],keys=perfAll_keys,color=mapColorStandard,isVal=isVal)
                    #set name file    
