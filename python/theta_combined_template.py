...
def get_full_model(input_file=input0H, signal='TpTp_M-*', histogram_filter=lambda w: True):
    ...
    for proc in procs:
        ...
        for obs in obsvs:
            if 'nW0_nB0' in obs:
                try: model.add_lognormal_uncertainty('top_control',  math.log(1.109), proc, obs) # from ttbar CR
                except RuntimeError: pass
            if 'nW0_nB1' in obs:
                try: model.add_lognormal_uncertainty('top_control',  math.log(1.045), proc, obs) # from ttbar CR
                except RuntimeError: pass
            if 'nW0_nB2' in obs:
                try: model.add_lognormal_uncertainty('top_control',  math.log(1.057), proc, obs) # from ttbar CR
                except RuntimeError: pass
            if 'nW0_nB3p' in obs:
                try: model.add_lognormal_uncertainty('top_control', math.log(1.057), proc, obs) # from ttbar CR
                except RuntimeError: pass
    ...
    for proc in procs:
        ...
        for obs in obsvs:
            if 'nW0_nB0' in obs:
                try: model.add_lognormal_uncertainty('ewk_control',  math.log(1.197), proc, obs) # from Wjets CR
                except RuntimeError: pass
            if 'nW0_nB1' in obs:
                try: model.add_lognormal_uncertainty('ewk_control',  math.log(1.197), proc, obs) # from Wjets CR
                except RuntimeError: pass
            ...
            if 'nW1p_nB0' in obs:
                try: model.add_lognormal_uncertainty('ewk_control', math.log(1.043), proc, obs) # from Wjets CR
                except RuntimeError: pass
            if 'nW1p_nB1' in obs:
                try: model.add_lognormal_uncertainty('ewk_control', math.log(1.043), proc, obs) # from Wjets CR
                except RuntimeError: pass
