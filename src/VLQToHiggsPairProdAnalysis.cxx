...
VLQToHiggsPairProdAnalysis::VLQToHiggsPairProdAnalysis(Context & ctx) {
    ...
    pre_modules.emplace_back(new JetTagCalculator(ctx, "n_btags", CSVBTag(btag_wp)));
    pre_modules.emplace_back(new TopTagCalculator(ctx.get_handle<int>("n_toptags"), CMSTopTag()));
